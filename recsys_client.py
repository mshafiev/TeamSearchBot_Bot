...
class RecSysClient:
    ...
    def get_recommendation(self, user_id: int, excluded: Optional[List[int]] = None) -> Optional[Dict[str, Any]]:
        """
        Получить рекомендацию пользователя через эндпоинт /recommend/{user_id}.

        Аргументы:
            user_id: ID пользователя, для которого ищем рекомендацию
            excluded: список ID пользователей, которых нужно исключить из рекомендаций

        Возвращает:
            Словарь с рекомендованным пользователем или None
        """
        params = {}
        if excluded:
            params["excluded"] = ",".join(str(e) for e in excluded)
        url = f"{self.base_url}/recommend/{user_id}"
        try:
            response = requests.get(url, params=params)
            response.raise_for_status()
            data = response.json()
            # Ожидается, что ответ будет {"recommendation": ...}
